<template>
  <scroll-bar>
    <div class="py-2">
      <menu-item v-for="(item, index) in staticMenuOption" :key="index" :label="item.label" :iconName="item.icon"
        @click="$router.push(item.path)" />
      <menu-item-group :label="staticPersonalMenuOption.label">
        <menu-item v-for="(item, index) in staticPersonalMenuOption.children" :key="index" :label="item.label"
          :iconName="item.iconName" @click="$router.push(item.path)" />
      </menu-item-group>
      <sub-menu :label="personalPlaylistsOption.label">
        <menu-item v-for="(item, index) in personalPlaylistsOption.children" :key="index" :label="item.label"
          iconName="love" @click="$router.push(item.key)" />
      </sub-menu>
      <sub-menu :label="favoredPlaylistsOption.label">
        <menu-item v-for="(item, index) in favoredPlaylistsOption.children" :key="index" :label="item.label"
          :iconName="item.iconName" @click="$router.push(item.key)" />
      </sub-menu>
    </div>
  </scroll-bar>
</template>
...
<script setup>
  ...
  const getUserPlaylists = async () => {
    if (userinfo.value) {
      let res = await api.playlist.getUserPlaylists(userinfo.value.id);
